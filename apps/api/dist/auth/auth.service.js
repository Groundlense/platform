...
let AuthService = class AuthService {
    ...
    async forgotPassword(input) {
        ...
        if (!email && !mobile) {
            throw new common_1.BadRequestException('Provide an email or mobile number');
        }
        const user = mobile
            ? await this.db.user.findFirst({ where: { mobile } })
            : await this.db.user.findUnique({ where: { email } });
        if (!user) {
            throw new common_1.NotFoundException(mobile
        ...
        let isMock = true;
        if (mobile) {
            isMock = !(process.env.TWILIO_ACCOUNT_SID &&
    ...
    async resetPassword(input) {
        ...
        const smsMock = type === 'MOBILE' &&
            !(process.env.TWILIO_ACCOUNT_SID &&
                process.env.TWILIO_AUTH_TOKEN &&
                process.env.TWILIO_PHONE_NUMBER);
        const codeOk = (record && record.code === code && record.expiresAt >= new Date()) ||
            (smsMock && code === '123456');
        if (!codeOk) {
