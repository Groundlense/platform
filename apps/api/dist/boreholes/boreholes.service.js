    ...
    async create(projectId, user, dto) {
        ...
        if (existing) {
            if (existing.status !== 'PLANNED') {
                throw new common_2.ForbiddenException(`Borehole ${dto.boreholeCode} is locked — fieldwork on it has started`);
            }
            ...
            await this.activityLogsService.log(user.id, 'BOREHOLE_UPDATED', 'BOREHOLE', updated.id);
            return updated;
        }
